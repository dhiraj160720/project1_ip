...
if(isset($_POST['submit'])){
  $Question= $conn->query("SELECT * FROM dbms_question order by question_id asc");
  while($rowques=mysqli_fetch_assoc($Question))
  {
    $question_id=$rowques['Question_id'];
    $b=$conn->query("SELECT * FROM dbms_answer where question_id=$question_id and correct=1 order by answer_id asc ");
  ...
  $sql1 = "INSERT into history (username,sub_name,marks,percentage) values ('$n1','DBMS','$marks','$percentage')";
...
<?php
  ...
  $marks=0;
  $ques_query="SELECT * FROM dbms_question order by question_id asc ";
  ...
  
 {
  ...
  $ques_query1="SELECT * FROM dbms_answer where question_id=$question_id";
  ...
  while($rowques1=mysqli_fetch_assoc($Answer))
    ...
    $sql="select `answer` from dbms_answer where `question_id`=$question_id and `correct`=1";//
